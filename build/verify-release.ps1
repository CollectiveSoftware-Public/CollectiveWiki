...
# Display only — never re-derive what gets verified from this. ConvertFrom-Json auto-coerces
# the signed ISO-8601 'Z' string to [datetime], and that value's default ToString() renders as a
# culture-formatted local-looking string with no timezone marker (e.g. "7/20/2026 2:00:00 PM"),
# which is not what was actually signed. Prefer the raw signed text; fall back to an explicit
# UTC render if the manifest shape ever changes underneath the regex.
$publishedDisplay = if ($rawJson -match '"published"\s*:\s*"([^"]*)"') { $Matches[1] } else { $manifest.published.ToUniversalTime().ToString('u') }
